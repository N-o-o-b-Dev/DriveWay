...
export function EditMaintenanceModal({ isOpen, onClose, record }) {
    const { updateMaintenanceRecord, cars, maintenanceRecords } = useDriveway()
    ...
    // Reset and populate when opening with a record
    useEffect(() => {
        if (isOpen && record) {
            ...
            // If the workshop exists in our consolidated list vs a one-off, logic handles it. 
            // We default to 'select' but if it's a unique edited name it will just show as selected value.
            setWorkshopMode('select')
        }
    }, [isOpen, record])

    // Extract unique workshops
    const existingWorkshops = maintenanceRecords.reduce((acc, rec) => {
        if (!acc.some(w => w.name === rec.workshopName)) {
            acc.push({
                name: rec.workshopName,
                details: rec.workshopDetails,
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!selectedCarId || !record) return
        ...
        >
            <div className="flex flex-col max-h-[90vh] w-full">
                ...
                <div className="flex items-center justify-between p-6 border-b border-white/5 bg-[#0F0F0F]">
                    <div>
                        <h2 className="text-xl font-bold text-white">Edit Maintenance Request</h2>
                        <p className="text-sm text-slate-400 mt-1">Update service ticket details.</p>
                    </div>
                </div>
                            ...
                            <div className="border border-dashed border-white/10 rounded-lg p-4 bg-[#1A1A1A]/50 hover:bg-[#1A1A1A] transition-colors">
                                {!formData.image ? (
                                    <div
                                        ...
                                        onClick={() => document.getElementById('edit-modal-maintenance-image').click()}
                                    >
                                ...
                                <input
                                    id="edit-modal-maintenance-image"
                                    type="file"
                                    accept="image/*"
                                    className="hidden"
